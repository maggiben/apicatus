...
exports.headers = function() { return headers; }

var invoke = require('request');
var url = require('url');
...
processResource = function(request, response, next) {
    var i;
    var url_parts = url.parse(request.url, true);
    var query = url_parts.query;
    for(i = 0; i < api.entries.length; i += 1) {
        if(api.entries[i].route.match(url_parts.pathname)) {
            var entry = api.entries[i];
            console.log("url matched: " + url_parts.pathname + " entry: " + entry.route);
            entry.hits += 1;
            if(true === entry.proxy.enabled) {
                return request.pipe(invoke(entry.proxy.url)).pipe(response)
            } else {
                var contentType = getById(entry.contentType.id, headers.contentTypes);
                response.contentType(contentType.label);
                var retJSON = JSON.stringify(entry.sends);
                return response.send(retJSON);
                //return response.sendfile(__dirname + entry.sends);
            }
        }
    }
}